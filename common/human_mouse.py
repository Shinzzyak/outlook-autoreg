...
# ---------------------------------------------------------------------------
# R25-F1d: human_press_and_hold_frame — versi frame-aware.
# page.mouse tidak menembus iframe; target HIP di dalam frame hsprotect.
# Koordinat cx,cy = viewport-absolute; konversi ke relatif frame.
# ---------------------------------------------------------------------------
async def human_press_and_hold_frame(page, frame, cx, cy, is_done=None, max_hold=8.0,
                                     min_hold=1.2, check_interval=0.5, start=None):
    """Same as human_press_and_hold but dispatches mouse events into `frame`.
    ...
    fx = cx - fb["x"]
    fy = cy - fb["y"]
    # 1) hover via locator di frame (native, menembus iframe)
    try:
        loc = frame.locator("#px-captcha").first
        if await loc.count() > 0:
            await loc.hover()
    except Exception:
        pass
    await asyncio.sleep(random.uniform(0.12, 0.35))
    # 2) down via frame.mouse (koordinat relatif frame)
    await frame.mouse.move(fx, fy)
    ...
    while True:
        ...
        if ti >= len(tre):
            ...
            ti = 0
        dx, dy = tre[ti]; ti += 1
        await frame.mouse.move(fx + dx, fy + dy)
        if is_done is not None and elapsed > min_hold and (elapsed - last_check) > check_interval:
            last_check = elapsed
            try:
                if await is_done():
    ...
    held = loop.time() - t0
    _debug(f"hold(frame) {held:.1f}s passed={passed}")
    return held, passed
