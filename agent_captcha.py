...
def ask_vision(prompt, image_b64, models=None, keys=None, max_tokens=300, temperature=0.0):
    ...
    if not keys:
        ...
        return None
    deadline = time.time() + 60  # P2-2: total budget 60s untuk semua model/key
    payload_msg = [{
        "role": "user",
        "content": [
    ...
    for model in models:
        for ki, key in enumerate(keys):
            if time.time() > deadline:  # P2-2: total deadline, jangan loop >60s
                print("  [vision] deadline exceeded, stopping")
                return None
            ...
            try:
                r = requests.post(
                    ...
                    timeout=30,  # P2-2: 120s per request terlalu lama
                )
                if r.status_code == 429:
                    time.sleep(1.5)  # P2-2: backoff singkat, jangan burn semua key
                    continue
